...
module Serialization =
    ...
    let private encodeShipmentEvent (event: ShipmentEvent) =
        ...
        | ShipmentPickedUp data -> "ShipmentPickedUp", Encode.object [ "pickedUpBy", Encode.string data.PickedUpBy ]
        | ContainerLoaded data -> "ContainerLoaded", Encode.object [ "containerId", encodeContainerId data.ContainerId ]
        | CustomsCleared data -> "CustomsCleared", Encode.object [ "clearedBy", encodeCustomsOffice data.ClearedBy ]
    ...
    let private eventIdDecoder: Decoder<EventId> = Decode.guid |> Decode.map EventId
    ...
    let private containerLoadedDecoder: Decoder<ContainerLoadedData> =
        Decode.object (fun get -> { ContainerId = get.Required.Field "containerId" containerIdDecoder })

    let private shipmentPickedUpDecoder: Decoder<ShipmentPickedUpData> =
        Decode.object (fun get -> { PickedUpBy = get.Required.Field "pickedUpBy" Decode.string })

    let private customsClearedDecoder: Decoder<CustomsClearedData> =
        Decode.object (fun get -> { ClearedBy = get.Required.Field "clearedBy" customsOfficeDecoder })
